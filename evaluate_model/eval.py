...
def evaluate_models():
    ...
    extended_output_size = int(
        max_output_size * EvaluationConfig.PREDICTION_LENGTH_MULTIPLIER
    )
    ...
    (
        ...
        input_size=max_input_size, output_size=extended_output_size
    )
        ...
        try:
            ...
            for i, series in enumerate(test_series):
                ...
                if len(series) >= model_info["input_size"] + extended_output_size:
                    ...
                    if model_name in ["tide", "nlinear", "linear_regression"]:
                        input_series_ext = input_series_ext.with_static_covariates(
                            test_static_cov[i]
                        )
                    test_inputs_extended.append(input_series_ext)

            # Get all predictions
            preds_standard = model_info["model"].predict(
                n=model_info["output_size"], series=test_inputs_standard
            )
            preds_extended = model_info["model"].predict(
                n=extended_output_size, series=test_inputs_extended
            )
