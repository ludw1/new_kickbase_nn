...
def _create_dummy_dataset(input_size: int, output_size: int, with_static_covariates: bool = False) -> TimeSeries:
    ...
    if with_static_covariates:
        # Create dummy static covariates - using 2 features as per typical setup
...
def load_model_from_checkpoint(model_name: str, model_path: str, encoders_path: Optional[str]) -> Tuple[Any, int, int]:
    ...
    if model_name == "nhits":
        ...
        # We need to pass a validation series to satisfy the early stopping callback
        logger.info("Initializing model with dummy data...")
        dummy_train = _create_dummy_dataset(input_size, output_size, with_static_covariates=True)
        dummy_val = _create_dummy_dataset(input_size, output_size, with_static_covariates=True)
        model.fit(dummy_train, val_series=dummy_val, epochs=1, verbose=False)
        ...
        if encoders_path and os.path.exists(encoders_path):
            ...
            try:
                ...
                loaded_encoders = torch.load(encoders_path, map_location=torch.device('cpu'), weights_only=False)
                ...
                if isinstance(loaded_encoders, dict):
                    ...
                    logger.warning("TiDE will use encoders fitted on dummy data - performance may be degraded!")
                else:
                    model.encoders = loaded_encoders
                    logger.info(f"Restored fitted encoders/scalers for TiDE model (type: {type(loaded_encoders).__name__})")
