...
class EvaluationConfig:
    ...
    MODEL_PATHS = {
        "nhits": os.path.join(
            PipelineConfig.DATA_DIR, "checkpoints", "nhits_state_dict.pt"
        ),
        "nlinear": os.path.join(
            PipelineConfig.DATA_DIR, "checkpoints", "nlinear_state_dict.pt"
        ),
        "tide": os.path.join(
            PipelineConfig.DATA_DIR, "checkpoints", "tide_state_dict.pt"
        ),
        "tide_encoders": os.path.join(
            PipelineConfig.DATA_DIR, "checkpoints", "tide_state_dict_encoders.pth"
        ),
        "linear_regression": os.path.join(
            PipelineConfig.DATA_DIR,
            "checkpoints",
            "linear_regression_kickbase_model_final.pth",
        ),
    }
    ...
    COMPARISON_TABLE_PATH = os.path.join(
        TrainingConfig.FIGS_DIR, "model_comparison_table.csv"
    )
    COMPARISON_PLOT_PATH = os.path.join(
        TrainingConfig.FIGS_DIR, "model_comparison_plot.png"
    )
    DETAILED_RESULTS_PATH = os.path.join(
        TrainingConfig.FIGS_DIR, "detailed_evaluation_results.csv"
    )
